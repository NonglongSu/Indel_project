#Version 2.1
#keep the sequence with "+++" even without any "---"
...
# ouDir  = "../test_human_mouse_rat/Data_6.2/Mafft/mixed_cds/"
...
main = function(inFile, ouDir, num1, num2){
  ...
  # keep the seqs with '+++' only.
    flag       = (length(dna) - 1):length(dna)
    Seq.set    = as.list(seq.set[flag])
    focal.name = name[flag]
    write.fasta(sequences = Seq.set, names = focal.name, nbchar = 80,
