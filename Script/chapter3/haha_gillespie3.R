...
D_onestep = function(r1,gdnor,dna0){
  dnaB = dna0
  tau  = 0
  ...
  for (iter in 1:500) {
    ...
    k=sample(c(3,6,9,12),1,prob=gdnor)
    if(length(dna)-pos<k){#cannot delete the end
      next
    }
...
I_onestep= function(r1,gdnor,dnaB1){
  tau = 0
...
main = function(ouDir, l, omega_z, ss){
  ...
  #phase rates:
  pha = c(0.6,0.15,0.25)   #use phase to simulate the {g0,g1,g2}
  r1  = g*pha*0.5
  ...
  ssize   = as.numeric(ss)
  for(i in 1:ssize){
    dnaB  = D_onestep(r1, gdnor,dna0)
    dnaB1 = S_onestep(r2,gtr,dnaB)
    dnaB2 = I_onestep(r1,gdnor,dnaB1)
