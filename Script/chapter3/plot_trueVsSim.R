...
#qqplot 
qqplt = function(tM,dM){
  colorblindP = "#CC6677"
  for(j in 1:ncol(tM)){
    qqplot(tM[,j],dM[,j],xlab=para[[j]], ylab="", main=NULL)
...
##true vs sim/im plot
plot_true_vs = function(tM,dM,ouD){
  ouF1=paste0(ouD,"/","qqplot.pdf")
  ouF2=paste0(ouD,"/","error_perc.pdf")
  
  
  #qqplot
  pdf(ouF1, onefile=TRUE, paper='a4', width=8, height=15, pointsize=18)
  par(mfrow=c(4,4),mai=c(1, 0.3, 0.1, 0.25), mgp=c(3,0.6,0))
  qqplt(tM,dM)
  dev.off()
...
main = function(targ,num){
  ...
  if(n==1){
    ...
    plot_true_vs(tMat[,5:nvar],dMat[,5:nvar],ouD1)
  }else{
    dMat    = readMatrix(inD2)
    plot_true_vs(tMat[,5:nvar],dMat[,5:nvar],ouD2)
  }
