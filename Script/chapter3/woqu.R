...
#Judge whether non-gap state would add the failed znz/zs
...
#non-syn:1 || syn:0
nogap_sum = function(As, rAs, syn, omegal, g){
...
# A = DNAStringSet(c("A---TCGATCGT","AAAAA---AAAA"))
###################################################################
ziqi_prob_wz = function(A,g0,e0,Pmat,codonstrs,syn,f0,wz0,omegal,norm.k1,norm.k2){
  ...
  rA     = rmgap2(A,As,g)
  rAs    = str_split(rA,'')
  M.012  = nogap_sum(As, rAs, syn, omegal, g)  #being modified
  M.trim = edge_trim(M.012, posEdt, omegal/3)          
  
  
  ##############################################
  #gap prob
  #scoreG = sum(N.012[,1]*log(g0*wz) + N.012[,2]*log(g0) + N.012[,3]*log(g0)) 
  scoreG = sum(N.012*log(g0)) + sum(N.012[,1]*log(wz0))
  
  #non-gap prob
  k1 = t(t(M.trim[,,1])*norm.k1)
  k2 = t(t(M.trim[,,2])*norm.k2)
  scoreM = sum(log(1-g0[1:3] + (1-wz0)*g0[1:3]*rowSums(k1)), na.rm=TRUE) +
           sum(log(1-g0[4:6] + (1-wz0)*g0[4:6]*rowSums(k2)), na.rm=TRUE) 
  ...
  #summary stat
  res = list(score_ziqi,N.012,avg.g,M.trim,dat,c(N_012),c(M_012)) ####test 
  return(res)
}
