...
##>>wocao
LL_min_Z2 = function(gv){
  g012 = gv[1:6]
  wz   = gv[7]
  #gap LL
  scoreG = sum(phasew*log(g012)) + sum(phasew[,1]*log(wz)) 
  #scoreG = sum(phasew*log(g012)) 
...
main = function(inD, ouF, omega_l){
  ...
  repeat{
    ...
    lk1     = (1-e0[1])*e0[1]^omebin
    lk2     = (1-e0[2])*e0[2]^omebin                    
    norm.k1 = lk1/sum(lk1)
    norm.k2 = lk2/sum(lk2)
    ...
    DD <<- datw
    pb = nmkb(fn=LL_min, par=p0, lower=0.0, upper=2.5, control=list(tol=1e-4,trace=T)) 
    if(pb$convergence != 0){
    ...
    ####test
    ##gap openning
    nnew = colSums(Wi*Gm)         ###vs rowSums(phasew)
    mnew = colSums(Wi*Mm)         ###vs rowSums(iw)
    gnew = nnew/(nnew+mnew)       ###vs gnew = rowSums(phasew)/(rowSums(phasew)+rowSums(iw))
    
    log(prod(g0^nnew)) 
    #vs
    sum(phasew*log(g0)) + sum(phasew[,1]*log(wz0)) 
    
    sum(log((1-g0)^mnew))
    #vs
    kk1 = rowSums(t(t(pw)*norm.k1))
    kk2 = rowSums(t(t(pw)*norm.k2))
    sum(iw[1:3,]*log(1-g0[1:3] + outer((1-wz0)*g0[1:3],kk1))) + 
    sum(iw[4:6,]*log(1-g0[4:6] + outer((1-wz0)*g0[4:6],kk2)))  
    ...
    gv  = c(g0,wz0)
    gb  = nmkb(fn=LL_min_Z2, par=gv, lower=0.0, upper=1.0, control=list(tol=1e-4,trace=T)) 
    if(gb$convergence != 0){
      cat("Warning: failed convergence!")
    }else{
      gv    = gb$par
    ...
    print(-log(1-(gnew[1]+gnew[4])))
    print(-log(1-(gnew[2]+gnew[5])))
    print(-log(1-(gnew[3]+gnew[6])))
    print(wznew)
    
    
    #rmse tolerance
    p    = c(g0,e0,p0,wz0)
    q    = c(gnew,enew,pnew,wznew)
    delta= (q-p)^2
